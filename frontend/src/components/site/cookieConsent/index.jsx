...
const actionClassName =
  "min-h-12 border border-ink/18 bg-transparent px-5 text-center text-sm font-medium text-ink transition-[border-color,color,transform] duration-180 ease-[cubic-bezier(.22,1,.36,1)] hover:border-primary/55 hover:text-primary active:scale-[.99]";
  ...
  return (
    <div className="grid grid-cols-[1fr_auto] gap-5 border-t border-ink/10 py-5 first:border-t-0">
      <div>
        ...
        <p className="mt-1.5 max-w-xl text-sm leading-6 text-ink/58">{description}</p>
      </div>
      <div className="flex flex-col items-end gap-2">
        <button
          aria-checked={checked}
          aria-label={title}
          className="relative mt-0.5 h-7 w-12 border border-ink/18 bg-ink/8 transition-colors duration-220 ease-[cubic-bezier(.22,1,.36,1)] aria-checked:border-primary aria-checked:bg-primary disabled:cursor-default disabled:opacity-55"
          disabled={disabled}
          onClick={() => onChange?.(!checked)}
          role="switch"
          type="button"
        >
          <span className="absolute start-0.5 top-0.5 grid size-5 translate-x-0 place-items-center bg-white text-primary shadow-[0_2px_8px_rgb(var(--shadow-rgb)_/_0.12)] transition-transform duration-260 ease-[cubic-bezier(.22,.68,.28,1)] rtl:aria-checked:-translate-x-5 ltr:aria-checked:translate-x-5">
            {disabled ? <Check aria-hidden="true" className="size-2.5" weight="bold" /> : null}
          </span>
        </button>
        <span className="text-xs font-semibold tracking-[0.08em] text-ink/58 uppercase">
          {status}
        </span>
      </div>
    </div>
      ...
      {showBanner ? (
        ...
        >
          <div className="flex items-start gap-4">
            <span className="grid size-10 shrink-0 place-items-center border border-primary/22 bg-primary/7 text-primary">
              <Cookie aria-hidden="true" className="size-5" weight="light" />
            </span>
            <div>
              <h2 id="cookie-consent-title" className="text-lg font-medium tracking-[-0.025em] text-ink">
                {labels.title}
              </h2>
              <p className="mt-1.5 text-sm leading-6 text-ink/60">{labels.description}</p>
            </div>
          </div>
        ...
        <DialogContent variant="responsive" className="flex flex-col bg-surface-overlay sm:max-w-[40rem]">
          <header className="flex shrink-0 items-start justify-between gap-5 border-b border-ink/12 px-5 py-5 sm:px-7 sm:py-6">
            <div>
              ...
              <DialogDescription className="mt-2 max-w-xl text-sm leading-6 text-ink/58">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 sm:px-7">
            ...
            <div className="border-t border-ink/12 py-6">
              <h3 className="text-base font-medium text-ink">{labels.detailsTitle}</h3>
              <div className="mt-4 grid gap-3">
                <div className="grid grid-cols-[auto_1fr] gap-x-4 border border-ink/10 p-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs text-ink/58">{labels.firstParty} · {labels.sixMonths}</p>
                  </div>
                </div>
                <div className="grid grid-cols-[auto_1fr] gap-x-4 border border-ink/10 p-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs text-ink/58">{labels.thirdParty} · {labels.providerDuration}</p>
                  </div>
                </div>
              </div>
            </div>
