...
export default async function Footer({ locale }) {
  ...
  return (
    <footer data-motion-reveal className="bg-[#37332e] text-[#f8f5ef]">
      <div className="grid-container">
        ...
        <div className="grid gap-10 py-11 sm:py-13 lg:grid-cols-[1.2fr_.8fr] lg:gap-16">
          <div>
            <p className="text-[0.66rem] font-semibold tracking-[0.16em] text-white/42 uppercase">
              {content.ui.contact}
            </p>
            <p className="mt-4 max-w-xl text-[clamp(2.35rem,5vw,4.25rem)] leading-[0.9] font-medium tracking-[-0.06em]">
              {siteConfig.name}
            </p>
            <address className="mt-7 max-w-md text-[0.82rem] leading-5.5 not-italic text-white/54">
              {siteConfig.address}
            </address>
          </div>
          ...
          <div className="grid content-start gap-8 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            <div>
              <p className="border-b border-white/12 pb-3 text-[0.64rem] font-semibold tracking-[0.14em] text-white/40 uppercase">
                {content.ui.call}
              </p>
              <div className="mt-3 grid">
                {siteConfig.phones.map((phone, index) => (
                  <a
                    key={phone.href}
                    href={phone.href}
                    className={`py-1.5 text-sm transition-colors duration-200 hover:text-white ${
                      index === 0 ? "text-white" : "text-white/52"
                    }`}
                  >
                    {phone.label}
                  </a>
        ...
        <div className="flex flex-col gap-3 border-t border-white/12 py-6 text-xs text-white/32 sm:flex-row sm:items-center sm:justify-between">
          <p>© {new Date().getFullYear()} {siteConfig.name}. {translations("rights")}</p>
