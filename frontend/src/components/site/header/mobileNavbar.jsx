...
function MenuGlyph({ open }) {
  return (
    <span aria-hidden="true" className="relative block h-4 w-5">
      <span className={`absolute start-0 top-[0.3rem] h-px w-5 bg-current transition-transform duration-400 ease-[cubic-bezier(.22,1,.36,1)] ${open ? "translate-y-[0.2rem] rotate-45" : ""}`} />
      <span className={`absolute start-0 bottom-[0.3rem] h-px w-5 bg-current transition-transform duration-400 ease-[cubic-bezier(.22,1,.36,1)] ${open ? "-translate-y-[0.2rem] -rotate-45" : ""}`} />
      <span className={`absolute start-0 bottom-[0rem] h-px w-5 bg-current transition-transform duration-400 ease-[cubic-bezier(.22,1,.36,1)] ${open ? "opacity-0" : ""}`} />
    </span>
  );
}
    ...
    >
      ...
      >
        ...
        <nav aria-label={labels.menu} className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain px-5 pb-7 pt-6 sm:px-7 sm:pb-9 sm:pt-8">
          <div>
            <div data-mobile-nav-item className="mb-4 text-[0.62rem] font-semibold tracking-[0.16em] text-ink/40 uppercase" style={{ "--mobile-nav-delay": "80ms" }}>
              <span>{labels.menu}</span>
            </div>

            <div data-mobile-nav-item className="border-t border-ink/12" style={{ "--mobile-nav-delay": "120ms" }}>
              <Link
                href="/"
            ...
            <div className="border-b border-ink/12 py-4">
              {groups.map((group, groupIndex) => (
                <details key={group.key} data-mobile-nav-item className="group border-t border-ink/10 first:border-t-0" style={{ "--mobile-nav-delay": `${170 + groupIndex * 45}ms` }}>
          ...
          <div data-mobile-nav-item className="mt-auto grid gap-5 border-t border-ink/12 pt-5" style={{ "--mobile-nav-delay": "360ms" }}>
